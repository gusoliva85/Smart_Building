...
- [x] **Backend: gestión de órdenes de trabajo (incluidas las manuales).**
  Mismo archivo `routers/ordentrabajo.py`. `POST /api/edificios/{id}/ordenes-trabajo` (gestión): OT manual, sin reclamo previo. `GET /api/edificios/{id}/ordenes-trabajo` (filtros `estado`/`tipo`/`prioridad`) y `GET /api/ordenes-trabajo/{id}`: listado y detalle, gestión únicamente — nunca el Propietario/Inquilino directo (su ventana sigue siendo el reclamo). `PATCH /api/ordenes-trabajo/{id}/asignacion`: asigna/reasigna `encargado_id` (validado contra un Usuario real con rol `encargado`, mismo criterio que `Edificio.encargado_id`) y/o `proveedor_id` suelto, con semántica `exclude_unset` (mandar `null` desasigna de verdad, no mandar el campo lo deja como está) — bloqueada una vez `resuelta` (el antecedente no se reescribe). `PATCH /api/ordenes-trabajo/{id}/estado`, usando `transicion_valida_ot()`: pasar a `en_curso` exige tener alguien asignado (`encargado_id` o `proveedor_id`, cualquiera de los dos alcanza) y fija `fecha_inicio`; pasar a `resuelta` fija `fecha_cierre`, acepta `costo` opcional, y dispara `sincronizar_reclamo_al_resolver_ot()` de verdad (la Tarea anterior la había dejado lista pero sin disparador real). **Ajuste real descubierto al conectar el disparador**: la transición de `Reclamo` de `asignado` directo a `resuelto` NO es válida (`TRANSICIONES_VALIDAS` exige pasar por `en_curso`) — por eso pasar la OT a `en_curso` también sincroniza el reclamo a `en_curso` (mismo mecanismo, `transicion_valida()`), así el reclamo camina en paralelo con su OT y la sincronización final a `resuelto` tiene efecto real en el camino feliz. `POST /api/ordenes-trabajo/{id}/evidencia`: agrega una `OtEvidencia` (antes/después). 20 tests nuevos en `test_ordenes_trabajo.py` (34 en total para el archivo, 310+34 en el backend completo).

- [x] **Backend: cálculo de tiempo de resolución.**
  Campo calculado (Documento Técnico sección 12; Documento General 10.5 y 11.7), por separado para reclamos y para órdenes de trabajo — el widget de KPIs que lo agrega/promedia es tarea de la Fase 6, acá solo se calcula por elemento. `services/reclamos.py`: `tiempo_resolucion_reclamo()`/`tiempo_resolucion_ot()`, lógica pura por duck typing (ningún import de modelos, sin ciclo con `models/reclamo.py`/`models/ordentrabajo.py` que sí importan este archivo). Para `OrdenTrabajo` ya alcanzaba con `fecha_cierre` (Tarea 6: `fecha_cierre - creado_en`). Para `Reclamo` hizo falta agregar **`cerrado_en`** (nullable, se fija una sola vez al llegar al estado terminal `cerrado`, `routers/reclamos.py::cambiar_estado_reclamo`) — 11.7 mide hasta el CIERRE, no hasta "resuelto" (que todavía puede reabrirse), así que `tiempo_resolucion_segundos` da `null` en un reclamo `resuelto` pero no `cerrado`. Ambos schemas de salida (`ReclamoSalida`/`OrdenTrabajoSalida`) exponen `tiempo_resolucion_segundos` como `@computed_field`. 11 tests nuevos (6 unitarios de servicio + 5 de integración).

- [x] **Backend: servicio de "peor estado" por departamento.**
  `services/severidad.py`: `reclamo_severity()`/`ot_severity()` (`snake_case`, no el `camelCase` del JS de referencia — la skill fija la REGLA, no el símbolo exacto), tal como quedaron documentados en la skill `premium-uiux` (`ot_severity()` solo puede devolver `ok` o `pend`, nunca `warn`/`crit` — y, a diferencia del mockup, sí filtra por estado acá mismo: Documento Técnico sección 12 dice que solo `en_curso` dispara el naranja, una OT todavía `pendiente` no cuenta) — el dato exacto que va a consumir el Dashboard Visual en la Fase 5. Alcance acotado a esta fase: `deuda_severity()` (financiero) y `severidad_maxima()`/"severidad según la vista" (que combinan las tres con la regla de precedencia) quedan para la Fase 5, cuando el Dashboard Visual arma la respuesta completa. Primer módulo con lógica de cálculo no trivial fuera de flujos de estado: 14 tests nuevos con pytest en esta misma tarea (Documento Técnico, sección 20).

- [x] **Frontend: pantalla de creación de reclamo.**
  `reclamos.html` + `reclamos.js` (nuevos), acceso de sidebar para Propietario/Inquilino únicamente (Administrador/Encargado suman el suyo en la próxima tarea, "seguimiento de reclamos" — mismo archivo, un dominio por archivo). Reutiliza `GET /api/mis-departamentos` (Fase 2) para el contexto: con una sola unidad propia el selector "¿sobre qué edificio?" ni se muestra (mismo criterio ya resuelto para Pagos); con más de una, un `<select>` de sus propias unidades. Objetivo del reclamo (unidad propia/espacio común/todo el edificio) y prioridad (leve/medio/crítico, con la explicación de Documento General 11.3) como tarjetas seleccionables nuevas (`.opcion-card`, agregada a `componentes.md`). Fotos como lista dinámica de URLs (agregar/quitar filas) — mismo criterio de "sin carga de archivo real" que el comprobante de un Pago, pero plural porque `Reclamo.fotos` sí lo es (una fila por foto desde la Tarea 2). Confirmación de envío con `.mensaje-exito` (patrón nuevo, agregado a `componentes.md`).
  **Gap real encontrado al construir esta pantalla**: `GET /api/edificios/{id}/espacios-comunes` era `admin`-only por error desde la Fase 1 — bloqueaba sin querer a Propietario/Inquilino/Encargado, que sí necesitan verla para reportar sobre un espacio común. Se corrigió reusando `requerir_acceso_para_crear_reclamo` (mismo criterio: gestión del edificio, o cualquiera con una unidad propia ahí); crear un espacio común sigue siendo exclusivo del administrador. 2 tests nuevos de backend.
  Probado de punta a punta con Playwright contra el backend real: alta con una y con varias unidades, objetivo "espacio común" (trae la lista real), envío exitoso, sin unidades asignadas (aviso bloqueante), acceso denegado para Administrador/Encargado/Auditor, tema oscuro y mobile — sin errores de consola en ningún caso.
